                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <div>
                                    <input class="form-control @error('enquiry_no') is-invalid @enderror" value="{{ old('enquiry_no') }}" type="text" name="enquiry_no" id="enquiry_no" placeholder="Enquiry Number..." required autofocus autocomplete="off">
                                    @error('enquiry_no')
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <div>
                                    <select class="form-control @error('employee_id') is-invalid @enderror" name="employee_id" id="employee_id" required autocomplete="off">
                                        @if(old('employee_id'))
                                            @php
                                                $employee = \App\Employee::find(old('employee_id'))
                                            @endphp
                                            <option value="{{ $employee->id }}">{{ $employee->cnic }} | {{ $employee->fullname }}</option>
                                        @endif
                                    </select>
                                    @error('employee_id')
                                    <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                    <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target=".modal-employee">Create</button>
                                </div>
                            </div>
                        </div>
                        <div class="col-6">
    ...
    <div class="modal fade modal-employee" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="employee_form" class="form-horizontal modal-form">
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary btn-create-employee">Create Employee</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(() => {
        ...
        $('#employee_id').select2({
            ...
            ajax: {
                ...
                processResults: function (data) {
                    return {
                        results: data.map(item => {
                            return {id: item.id, text: (item.cnic == null ? '00000-0000000-0' : item.cnic) + " | " + item.fullname}
                        })
                    };
                }
            }
        ...
        $(document.body).on("click", ".btn-create-employee",(evt) => {
            $("#employee_form .invalid-feedback").remove();
            $.ajax({
